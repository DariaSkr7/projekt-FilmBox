...
const id = document.location.hash.slice(1, document.location.hash.length);
console.log(id);
...
document.innerHTML += `<h3>${filmy.nazev}</h3>
	<p>${filmy.popis}</p>
	<p>${filmy.plakat}</p>
`;

const titleElement = document.querySelector('#detail-filmu .card-title');
titleElement.textContent = film.nazev;

const popisElement = document.querySelector('#detail-filmu .card-text');
popisElement.textContent = film.popis;

const imgElement = document.querySelector('#detail-filmu div div img');
imgElement.src = film.plakat.url;

const daysLeft = dayjs(film.premiera).diff(dayjs(), 'days');

let correctForm = `${daysLeft} dni`;
if (daysLeft === 1) {
  correctForm = '1 den';
}
const premieraElement = document.querySelector('#premiera');
popisElement.innerHTML = `Premiéra <strong>${dayjs(film.premiera).format(
  'D. M. YYYY',
)}</strong> což je za ${correctForm}`;
...
noteForm.addEventListener('submit', (event) => {
  ...
  if (input.value === '') {
    ...
    if (!checkbox.checked) {
      ...
      noteForm.innerHTML = `<p class="card-text">${input.value}</p>`;
    }
  }
});

const control = document.querySelector('#prehravac');
...
if (control) {
  ...
  const showControl = () => {
    clearTimeout(timer);
    timer = setTimeout(hideControl, 3000);
  ...
  document.addEventListener('keydown', showControl);
}
